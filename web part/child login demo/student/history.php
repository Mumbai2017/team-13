...
<div align="center">
    ...
    <table cellpadding="1" cellspacing="1">
		...
		$sql_student = mysqli_query($connect, "select * from student where rollno='$rollno'");
		$row_student = mysqli_fetch_array($sql_student);
		$elective = $row_student['EL_1'];
		...
		$sql_coursemapping='SELECT * FROM coursemapping WHERE Year="'.$year.'" AND Division="'.$division.'"';
		$res_coursemapping=mysqli_query($connect, $sql_coursemapping);
		//echo $sql_coursemapping;
		while($row_coursemapping=mysqli_fetch_array($res_coursemapping))
		{
			array_push($subs, $row_coursemapping['Subject_code']);
		...
		{
			if($row_analysis['Elective'] == 'E')
			{
				if($row_analysis['Subject_code'] != $elective)
					continue;
			}
			if(in_array($row_analysis['Subject_code'], $subs))
			{
